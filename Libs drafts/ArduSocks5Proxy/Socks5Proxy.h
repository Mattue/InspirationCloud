...
public:
    Socks5Proxy(IPAddress proxyIP, uint16_t proxyPort, Client &client)
        : client(client), socks5Host(proxyIP), socks5Port(proxyPort)
    {
    }
    ...
    virtual int connect(const char *destHost, uint16_t destPort)
    {
        ...
        // int s5ConnectResult = client.connect(socks5Host, socks5Port);
        // int s5ConnectResult = client.connect("proxy_user:qwerty1480@176.223.142.19:1080", socks5Port);
        int s5ConnectResult = client.connect("socks5h://proxy_user:qwerty1480@176.223.142.19:1080", socks5Port);
        if (!s5ConnectResult)
        {
            return s5ConnectResult;
        }
...
protected:
    ...
    {
        if (!Socks5Auth() || !Socks5ConnectByHost(destHost, destPort))
        {
            return 0;
        }
    ...
    bool Socks5Auth()
    {
        ...
        if (ReadAll(client, (uint8_t *)&s5AResp, sizeof(TSocks5AuthResponse)) != sizeof(TSocks5AuthResponse) || s5AResp.VER != 0x05 || s5AResp.METHOD != 0x00)
        {
    ...
    int Socks5ConnectResponse()
    {
        ...
        if (ReadAll(client, (uint8_t *)&s5CResp, sizeof(Socks5ConnResponse)) != sizeof(Socks5ConnResponse) || s5CResp.VER != 0x05 || s5CResp.REP != 0x00 || s5CResp.ATYP != 0x01)
        {
    ...
    bool Socks5ConnectByIp(IPAddress destIp, uint16_t destPort)
    {
        ...
        Serial.println("");
        if (WriteAll(client, (uint8_t *)&s5CReq, sizeof(s5CReq)) != sizeof(s5CReq) || !WaitClient(client))
        {
            return false;
        }
        return true;
